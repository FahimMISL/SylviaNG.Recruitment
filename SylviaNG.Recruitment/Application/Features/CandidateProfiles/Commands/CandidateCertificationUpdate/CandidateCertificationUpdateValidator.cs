...
{
    public class CandidateCertificationUpdateValidator : AbstractValidator<CandidateCertificationUpdateCommand>
    {
        public CandidateCertificationUpdateValidator(IOptions<FileStorageSettings> fileStorageOptions)
        {
            var settings = fileStorageOptions.Value;
            var maxFileSizeBytes = settings.MaxFileSizeMB * 1024L * 1024L;

            RuleFor(x => x.CandidateCertificationId)
                .GreaterThan(0).WithMessage("CandidateCertificationId is required.");
