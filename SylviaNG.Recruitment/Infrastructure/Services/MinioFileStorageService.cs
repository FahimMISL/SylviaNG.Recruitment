...
{
    /// <summary>
    /// MinIO (S3-compatible) implementation of <see cref="IFileStorageService"/>. Object key
    /// mirrors the local-disk relative path shape ("job-postings/{subFolder}/{guid}{ext}") so
    /// DB-stored values behave the same regardless of which FileStorage:Provider is active.
    /// Shares one bucket with <see cref="MinioApplicationCvStorageService"/>, separated by key
    /// prefix - same relationship as the two Local implementations' sibling wwwroot folders.
    /// </summary>
    public class MinioFileStorageService : IFileStorageService
    {
        private const string RootPrefix = "job-postings";
        private static readonly FileExtensionContentTypeProvider ContentTypeProvider = new();

        private readonly IMinioClient _minioClient;
        private readonly MinioSettings _settings;

        public MinioFileStorageService(IMinioClient minioClient, IOptions<MinioSettings> options)
        {
