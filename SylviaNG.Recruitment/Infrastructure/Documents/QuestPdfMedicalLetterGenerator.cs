...
{
    /// <summary>
    /// EP-18 F3: branded medical referral letter (EP-10 US-086), rolled onto
    /// BrandedLetterPdfComposer - same "letter" shape as QuestPdfOfferLetterGenerator.
    /// </summary>
    public class QuestPdfMedicalLetterGenerator : IMedicalLetterPdfGeneratorService
    {
        ...
        public QuestPdfMedicalLetterGenerator(IWebHostEnvironment environment, IBrandingResolverService brandingResolverService)
        {
        ...
        public async Task<byte[]> Generate(string documentTitle, string candidateName, string finalBody, long sequenceValue)
        {
            ...
            var referenceNumber = ReferenceNumberComponent.BuildReferenceNumber(branding, "MEDICAL", issueDate.Year, sequenceValue);
